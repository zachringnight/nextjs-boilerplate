...
export default function PlayerAvatar({ player, size = 'md' }: PlayerAvatarProps) {
  ...
  // Use UI Avatars as placeholder with position-based colors
  const placeholderUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(player.firstName + '+' + player.lastName)}&background=${colors.bg}&color=fff&size=${sizePx[size] * 2}&bold=true&format=svg`;

...
function StationsInner() {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- intentional: mark client hydrated for SSR-safe time rendering
    setClientReady(true);
  }, []);
      ...
      <div className="p-4 space-y-3">
        {stationSchedule.length === 0 ? (
          ...
          stationSchedule.map((slot) => {
            ...
            const isCurrent = clientReady && isCurrentSlot(slot.date, slot.startTime, slot.endTime);
            const isPast = clientReady && isPastSlot(slot.date, slot.endTime);
...
export default function StationsContent() {
  return (
    <Suspense fallback={<PageSkeleton type="live" />}>
      <StationsInner />
    </Suspense>
  );
}

...
export default function QuickClipModal() {
  ...
  // Auto-generate clip name (derived, not an effect)
  ...
  // Apply auto-generated name when user hasn't manually edited it
  const displayedClipName = isEditMode || userEditedName ? clipName : autoGeneratedName;

  // Populate form when modal opens (state adjustment during render, per React docs)
  const prevModalKeyRef = useRef<string | null>(null);
  const modalKey = clipModalOpen ? `${editingClipId ?? 'new'}` : null;
  ...
  }, [setClipModalOpen, setEditingClipId]);
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/80 flex items-end sm:items-center justify-center" onClick={handleClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={isEditMode ? 'Edit Clip' : 'New Clip'}
        tabIndex={-1}
        className="bg-[#1A1A1A] w-full sm:max-w-lg sm:rounded-2xl rounded-t-2xl border border-[#2A2A2A] max-h-[90vh] overflow-y-auto outline-none"

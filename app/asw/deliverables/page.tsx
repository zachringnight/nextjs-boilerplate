...
export default function DeliverablesPage() {
  ...
  const filteredDeliverables = useMemo(() => {
    ...
    if (selectedDay !== 'all') result = result.filter((d) => d.dueDay === selectedDay);
    ...
    <div className="min-h-screen bg-zinc-950 pb-24">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className={`text-zinc-400 mt-1 ${largeText ? 'text-base' : 'text-sm'}`}>
              Tunnel walks, product photos & daily recaps
            </p>
          </div>
          <button
            onClick={() => {
              if (confirm('Reset all deliverables to pending?')) {
                resetDeliverables();
              }
            }}
      ...
      <div className="grid grid-cols-4 gap-2 p-4">
        <div className="bg-zinc-900 rounded-lg p-3 text-center">
          <div className={`font-bold text-zinc-400 ${largeText ? 'text-2xl' : 'text-xl'}`}>{progress.pending}</div>
          <div className={`text-zinc-500 ${largeText ? 'text-sm' : 'text-xs'}`}>Pending</div>
        </div>
        <div className="bg-amber-900/30 rounded-lg p-3 text-center">
          <div className={`font-bold text-amber-400 ${largeText ? 'text-2xl' : 'text-xl'}`}>{progress.inProgress}</div>
          <div className={`text-amber-500/80 ${largeText ? 'text-sm' : 'text-xs'}`}>In Progress</div>
        </div>
        <div className="bg-green-900/30 rounded-lg p-3 text-center">
          <div className={`font-bold text-green-400 ${largeText ? 'text-2xl' : 'text-xl'}`}>{progress.completed}</div>
          <div className={`text-green-500/80 ${largeText ? 'text-sm' : 'text-xs'}`}>Completed</div>
        </div>
        <div className="bg-blue-900/30 rounded-lg p-3 text-center">
          <div className={`font-bold text-blue-400 ${largeText ? 'text-2xl' : 'text-xl'}`}>{progress.delivered}</div>
          <div className={`text-blue-500/80 ${largeText ? 'text-sm' : 'text-xs'}`}>Delivered</div>
        </div>
      </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`py-2 px-4 rounded-lg font-medium text-sm transition-colors ${
              selectedDay === 'all' ? 'bg-zinc-700 text-white' : 'bg-zinc-900 text-zinc-400 hover:bg-zinc-800'
            }`}
          >
            All Days
          </button>
          {EVENT_DAYS.map((day) => (
            <button
              ...
              className={`py-2 px-4 rounded-lg font-medium text-sm transition-colors ${
                selectedDay === day ? `${dayColors[day]} text-white` : 'bg-zinc-900 text-zinc-400 hover:bg-zinc-800'
              }`}
            >
              {day.slice(0, 3)}
            </button>
        ...
        <div className="mx-4 mb-4 bg-zinc-900 border border-zinc-800 rounded-lg p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => setNewDeliverable({ ...newDeliverable, type: e.target.value as DeliverableType })}
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setNewDeliverable({ ...newDeliverable, dueDay: e.target.value as EventDay })}
                className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-white"
              >
                {EVENT_DAYS.map((day) => (
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setNewDeliverable({ ...newDeliverable, playerId: e.target.value })}
              className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-white"
            >
      ...
      <div className="px-4 space-y-2">
        {filteredDeliverables.length === 0 ? (
          ...
          filteredDeliverables.map((deliverable) => {
            ...
            return (
              <div key={deliverable.id} className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <div className={`font-medium text-white ${largeText ? 'text-lg' : 'text-base'}`}>{deliverable.title}</div>
                        {deliverable.description && (
                          <div className={`text-zinc-500 mt-0.5 ${largeText ? 'text-base' : 'text-sm'}`}>
                            {deliverable.description}
                          </div>
                        )}
                      </div>
                    ...
                    {(deliverable.completedAt || deliverable.deliveredAt) && (
                      <div className={`text-zinc-600 mt-2 ${largeText ? 'text-sm' : 'text-xs'}`}>

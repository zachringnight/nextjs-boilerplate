...
import { fetchPartnerPartnerships } from '../lib/queries';
import { deletePartnerPartnership } from '../lib/mutations';
import { formatDate } from '../lib/utils';
import type { PartnerPartnership } from '../types';

export default function PartnerPartnershipsPage() {
  ...
  const [editItem, setEditItem] = useState<PartnerPartnership | null>(null);
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      setPartnerships(await fetchPartnerPartnerships());
    } catch (e) {
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await deletePartnerPartnership(deleteId);
      triggerRefresh();
      setDeleteId(null);
      ...
      <main className="flex-1 pb-20 md:pb-0">
        <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Partner Partnerships</h1>
            <button
              ...
              {partnerships.map((p) => (
                <div key={p.id} className="bg-[#141414] border border-[#2A2A2A] rounded-xl p-4 group">
                  <div className="flex items-start justify-between">
                    <h3 className="text-white font-semibold text-sm">{p.partner_name}</h3>
        ...
        <ConfirmModal
          ...
          message="This will permanently delete this partner partnership."
          loading={deleting}
        />
      </main>
